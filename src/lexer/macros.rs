    ...
    ($char: literal => $one: ident || $two: ident; $self: ident) => {
        {
            if $self.cursor.lookup(1) == $char {
                $self.cursor.next();
                $self.cursor.next();
                Ok(crate::lexer::token::Token::new($self.cursor.chunk(), crate::lexer::token::TokenKind::$one))
            } else {
                $self.cursor.next();
                Ok(crate::lexer::token::Token::new($self.cursor.chunk(), crate::lexer::token::TokenKind::$two))
            }
        }
